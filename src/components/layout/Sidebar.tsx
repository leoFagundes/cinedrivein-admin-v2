...
const navItems: NavItem[] = [
  { label: "Dashboard",             href: "/admin/dashboard", icon: <FiGrid size={18} /> },
  { label: "Pedidos",               href: "/admin/orders",    icon: <FiShoppingBag size={18} /> },
  { label: "Estoque",               href: "/admin/stock",     icon: <FiBox size={18} /> },
  { label: "Usuários",              href: "/admin/users",     icon: <FiUsers size={18} /> },
  { label: "Logs",                  href: "/admin/logs",      icon: <FiActivity size={18} /> },
  { label: "Configurações do Site", href: "/admin/site",      icon: <FiGlobe size={18} /> },
];
...
function NavLink({ item, onClick }: { item: NavItem; onClick?: () => void }) {
  ...
  return (
    <Link
      ...
      onMouseEnter={(e) => {
        if (!active) e.currentTarget.style.backgroundColor = "var(--color-bg-elevated)";
      }}
      onMouseLeave={(e) => {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 flex-shrink-0" style={{ borderTop: "1px solid var(--color-border)" }}>
        <div
          className="flex items-center gap-3 px-3 py-2 rounded-[var(--radius-md)] mb-2"
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-40 flex">
          ...
          >
            <button
              onClick={close}
              className="absolute top-4 right-4 p-1.5 rounded-[var(--radius-sm)] cursor-pointer"
              style={{ color: "var(--color-text-muted)" }}
